...
import { Search, MapPin, CalendarClock, Pill, Clock, AlertCircle, List, TrendingUp, Home as HomeIcon, Navigation } from "lucide-react";
    ...
    <div className={`min-h-screen pb-20 font-sans transition-colors duration-300 ${isSeniorMode ? 'bg-amber-50/40' : 'bg-slate-50'}`}>
      ...
      <div className={`p-4 text-center border-b transition-colors duration-300 ${isSeniorMode ? 'bg-amber-100 text-amber-950 border-amber-200' : 'bg-slate-100 text-slate-700 border-slate-200'}`}>
        <div className="max-w-md mx-auto flex items-center justify-between gap-3 px-1">
          ...
          <button 
            ...
            className={`px-4 py-2 rounded-2xl font-black transition-all shadow-md active:scale-95 border-none cursor-pointer ${
              isSeniorMode 
      ...
      <header className={`bg-emerald-600 text-white p-6 shadow-md rounded-b-3xl ${isSeniorMode ? 'py-8' : ''}`}>
        <div className="max-w-md mx-auto text-center sm:text-left">
          ...
          <p className={`mt-2 text-emerald-100 ${isSeniorMode ? 'text-lg font-bold' : 'text-sm'}`}>Encuentra tu farmacia rápido y fácil.</p>
        </div>
      </header>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <Input 
                  ...
                  className={`pl-12 rounded-2xl shadow-sm transition-all bg-white ${
                    isSeniorMode 
              ...
              <div className="mt-6 animate-in fade-in duration-500">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {['Paracetamol', 'Losartán', 'Ibuprofeno', 'Ketorolaco', 'Zopiclona', 'Clorfenamina'].map(med => (
                    <Card 
                      key={med} 
                      className={`cursor-pointer border transition-colors shadow-sm ${
                        isSeniorMode 
              ...
              <div className="space-y-3 mt-4">
                ...
                {medicamentosBuscados.length > 0 ? (
                  medicamentosBuscados.map((item: any, idx) => (
                    <Card 
                      key={idx} 
                      className={`rounded-2xl border-none shadow-md overflow-hidden hover:shadow-lg transition-shadow bg-white ${
                        isSeniorMode ? 'border-4 border-emerald-600 shadow-xl' : ''
                      }`}
                    >
                  ...
                  >
                    ...
                    return (
                      <Dialog key={f.id}>
                        ...
                        >
                          <div className={`rounded-full border-2 border-white shadow-lg transition-transform group-hover:scale-125 ${
                            isSeniorMode ? 'w-8 h-8' : 'w-5 h-5'
                          } ${f.abierto ? 'bg-emerald-500' : 'bg-red-400'} ${f.deTurno ? 'animate-pulse ring-4 ring-emerald-200' : ''}`} />
                          <div className={`bg-white font-bold rounded-full shadow-md whitespace-nowrap opacity-90 group-hover:opacity-100 text-gray-700 border border-gray-100 ${
                            isSeniorMode ? 'text-xs px-2.5 py-1 font-black mt-2 border-2 border-slate-300' : 'text-[10px] px-2 py-0.5 mt-1'
                          }`}>
                        ...
                        <DialogContent className="rounded-3xl sm:max-w-md w-[90%]">
                          ...
                          <div className="space-y-4 py-4">
                            ...
                            <Button 
                              className={`w-full mt-4 bg-emerald-600 hover:bg-emerald-700 rounded-xl shadow-lg border-none cursor-pointer text-white flex items-center justify-center ${isSeniorMode ? 'h-16 text-lg font-black' : 'h-12 text-md'}`}
          ...
          <TabsContent value="list" className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="space-y-4">
              ...
              {farmacias.map((f) => (
                <Card 
                  key={f.id} 
                  className={`rounded-2xl border-none shadow-sm hover:shadow-md transition-shadow bg-white ${
                    isSeniorMode ? 'border-4 border-emerald-600 shadow-xl' : ''
                  }`}
                >
              ...
              <div className="space-y-5">
                <div className={`bg-emerald-600 text-white rounded-3xl p-6 shadow-xl flex items-center justify-between relative overflow-hidden ${isSeniorMode ? 'border-4 border-yellow-400 p-8' : ''}`}>
                  ...
                  <div className="relative z-10 w-full">
                    ...
                    <Button 
                      className={`mt-4 font-bold bg-white hover:bg-emerald-50 shadow-md border-none cursor-pointer text-emerald-800 ${
                        isSeniorMode ? 'w-full h-16 text-lg font-black rounded-2xl' : 'size-sm rounded-xl'
                      }`} 
                      onClick={() => setActiveTab("map")}
                    >
                  ...
                  <div className="space-y-3">
                    {nextTurnos.map((turno, i) => (
                      <Card 
                        key={i} 
                        className={`rounded-2xl border-none shadow-sm hover:shadow-md transition-shadow bg-white ${
                          isSeniorMode ? 'border-4 border-emerald-600 shadow-xl p-2' : ''
                        }`}
                      >
                        <CardContent className="p-4 flex gap-4 items-center bg-white">
                          <div className={`text-emerald-800 rounded-xl flex flex-col items-center justify-center border border-emerald-100 ${
                            isSeniorMode ? 'bg-amber-100/70 p-4 border-2 border-emerald-600 min-w-[85px]' : 'bg-emerald-50 p-3 min-w-[65px]'
                          }`}>
